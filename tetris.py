...
Simple Pygame implementation of the classic Tetris game.
...
You can bundle the game into a single runnable file with [PyInstaller](https://pyinstaller.org/).

1. Install dependencies (PyGame and PyInstaller):

   ```bash
   python -m pip install -r requirements.txt
   ```

2. Run the provided helper script. It cleans previous build artifacts and creates a single-file executable in `dist/`.

   ```bash
   ./build_executable.sh
   ```

On Windows the produced binary will be `dist/tetris.exe`; on macOS/Linux it will be `dist/tetris`.

> Note: A prebuilt executable is not committed because this environment cannot download the
> required packages (outbound network is blocked). Run the steps above on a machine with
> internet access to produce `dist/tetris.exe`.
build_executable.sh
신규
+10
-0
...
PyInstaller build could not be produced in this environment because outbound network access is blocked.
Install dependencies (pygame==2.5.2, pyinstaller==6.11.1) in an environment with internet access, then run:

    ./build_executable.sh

This will generate a standalone tetris executable in the dist/ directory (tetris.exe on Windows).
requirements.txt
신규
+2
-0
